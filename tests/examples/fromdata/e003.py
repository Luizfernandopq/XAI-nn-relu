...
if __name__ == '__main__':
    ...
    trainer = ForwardReluTrainer(explanator.network, train_loader=None)
    trainer.update_loaders(train_set, test_set)
    trainer.fit(250)

    x, y = explanator.dataset[0]
    print(x, y)
    values = explanator.network.get_all_neuron_values(x)
    for value in values:
        print(pd.DataFrame(value, columns=["neurons"]), end="\n\n")