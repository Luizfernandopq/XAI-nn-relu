...
def run(layers):
    ...
    ok = 0
    notok = 0
    for i in range(int(test_set.__len__()/2000)):
        instance, y_true = test_set[i]
        ...
        importancias = []
        for j in inputs:
            index_input = int(j.name.split("input_")[1]) - 1
            explication[index_input] = 1.0
            importancias.append(index_input)
